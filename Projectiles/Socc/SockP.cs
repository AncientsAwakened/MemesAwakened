...
{
	public class SockP : ModProjectile
	{
		...
		public override void Kill(int timeLeft)
		{
			...
			for (int h = 0; h < 5; h++)
			{
				...
				vel *= 5f;
				int p = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, vel.X, vel.Y, 249, projectile.damage/2, 0, Main.myPlayer);
				Main.projectile[p].ranged = false;
				Main.projectile[p].thrown = true;
				Main.projectile[p].usesLocalNPCImmunity = true;
				Main.projectile[p].localNPCHitCooldown = 2;
			}
